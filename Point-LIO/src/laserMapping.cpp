...
void LaserMappingNode::readParameters()
{
    ...
    this->declare_parameter<std::string>("common.odom_frame", "livox");
    ...
    this->declare_parameter<vector<double> >("init_pos.init_trans", vector<double>{0.0, 0.0, 0.0});
    this->declare_parameter<vector<double> >("init_pos.initial_rot", vector<double>{0.0, 0.0, 0.0, 1.0});
    ...
    this->get_parameter_or<vector<double> >("init_pos.init_trans", initial_trans,
                                            vector<double>{0.0, 0.0, 0.0});
    this->get_parameter_or<vector<double> >("init_pos.initial_rot", initial_rot,
...
bool LaserMappingNode::sync_packages(MeasureGroup &meas)
{
    if (!imu_en) //不启用imu
    {
        if (!lidar_buffer.empty())
        {
            ...
            {
                RCLCPP_ERROR(get_logger(), "sync_packages: lose lidar");
                return false;
            }
    ...
    if(!lidar_pushed)
    {
        ...
        if(meas.lidar->points.size() < 1)
        {
            RCLCPP_ERROR(get_logger(), "lose lidar");
            lidar_buffer.pop_front();
            time_buffer.pop_front();
            return false;
        }
...
void LaserMappingNode::publish_obstacle_frame_body(rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr pubLaserCloudObstacle_body)
{
    ...
    laserCloudmsg.header.stamp = get_ros_time(lidar_end_time);
    laserCloudmsg.header.frame_id = odom_frame;  //原map_frame
    ...
    if (!use_imu_as_input)
    {
        if(is_first_kf_)
        {
            if(task == "A")
            {
                ...
                RCLCPP_INFO(this->get_logger(), "use initial pos: %f, %f, %f",
                ...
                RCLCPP_INFO(this->get_logger(), "use initial rot(wxyz): %f, %f, %f, %f",
                    initial_rot[3], initial_rot[0], initial_rot[1], initial_rot[2]);
            ...
            }else if(task == "B")
            {
                ...
                kf_output.x_.pos(1) = init_y_;
                kf_output.x_.pos(2) = init_z_;
                RCLCPP_INFO(this->get_logger(), "use initial pos: %f, %f, %f", init_x_, init_y_, init_z_);
                out.position.x = kf_output.x_.pos(0);
                out.position.y = kf_output.x_.pos(1);
                out.position.z = kf_output.x_.pos(2);
...
void LaserMappingNode::timer_callback()
{
    if (sync_packages(Measures)) //获取新一组待处理Measures
    {
        if (flg_first_scan) { //雷达第一帧
            init_x_ = initial_trans[0];
            init_y_ = initial_trans[1];
            init_z_ = initial_trans[2];

            first_lidar_time = Measures.lidar_beg_time;
            flg_first_scan = false;
            RCLCPP_INFO(this->get_logger(), "first lidar time %f", first_lidar_time);
        }
        if (flg_reset) {
            RCLCPP_INFO(this->get_logger(), "reset when rosbag play back");
            p_imu->Reset();
            flg_reset = false;
            return;
        }
        ...
        t5 = omp_get_wtime();
        if (path_en) publish_path(pubPath); //TODO 给决策路径点消息
        ...
        if (runtime_pos_log) {
            ...
            time_log_counter++;
            printf(
                "[ mapping ]: time: IMU + Map + Input Downsample: %0.6f ave match: %0.6f ave solve: %0.6f  ave ICP: %0.6f  map incre: %0.6f ave total: %0.6f icp: %0.6f propogate: %0.6f \n",
                t1 - t0, aver_time_match, aver_time_solve, t3 - t1, t5 - t3, aver_time_consu, aver_time_icp,
                aver_time_propag); //打印调试信息。
            if (!publish_odometry_without_downsample) {
                if (!use_imu_as_input) {
                    state_out = kf_output.x_;
                    euler_cur = SO3ToEuler(state_out.rot);
